...
func MakeHttpHandler(ctx context.Context, endpoint ArithmeticEndpoint, logger log.Logger) http.Handler {
	r := mux.NewRouter()
	...
	r.Methods("POST").Path("/test/{type}/{a}/{b}").Handler(kithttp.NewServer(
